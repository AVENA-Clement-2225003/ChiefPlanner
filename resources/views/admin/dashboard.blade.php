                        ...
                        <tr>
                            ...
                            @if($account->id_role !== 0)
                                <form method="post" action="{{ route('admin.user.changeRole', ['user_id' => $account->id_utilisateur]) }}">
                                    ...
                                    <td class="actions">
                                        <button type="submit" class="admin-action-button btn-success save" title="Enregistrer">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-check" viewBox="0 0 16 16">
                                                <path d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425z"/>
                                            </svg>
                                        </button>
                                        <button type="button" class="admin-action-button btn-danger delete" onclick="deleteUser({{ $account->id_utilisateur }}, '{{ $account->nom }}')" title="Supprimer">
